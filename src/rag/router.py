...
BGE_MODEL_PATH = os.getenv("EMBEDDING_MODEL_PATH")
EMBED_DIM = 1024
...
@app.post("/sync/scenic", response_model=ScenicSyncResponse)
async def sync_scenic(payload: ScenicSyncPayload):
    """Receive one scenic-area payload from A side and upsert structured RAG data."""
    try:
...
@app.get("/sync/jobs/{job_id}", response_model=SyncJobStatusResponse)
async def sync_job_status(job_id: str):
    """Return one-click sync job status and recent events."""

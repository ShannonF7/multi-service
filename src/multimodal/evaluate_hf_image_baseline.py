"""Evaluate local HuggingFace image encoders on the curated image-node dataset.

The script keeps vectors in the output directory instead of writing to the
production image_embeddings table. It supports CLIP-like models exposing
get_image_features, including CLIP, Chinese-CLIP and SigLIP variants in
Transformers.
"""
...
class LocalHFImageEncoder:
    def __init__(self, model_path: str, device: str | None = None):
        self.model_path = model_path
        self.device = device or ("cuda" if torch.cuda.is_available() else "cpu")
        self.processor = AutoImageProcessor.from_pretrained(model_path, local_files_only=True, trust_remote_code=True)
    ...
    def encode_image(self, image_path: str) -> list[float]:
        ...
        with torch.no_grad():
            if hasattr(self.model, "get_image_features"):
                ...
                if features is None:
                    raise RuntimeError(f"model at {self.model_path} does not expose image features")
        features = features / features.norm(dim=-1, keepdim=True)
        return [float(item) for item in features.detach().cpu().numpy().flatten().tolist()]
...
def load_or_create_vectors(
    ...
    if needed:
        encoder = LocalHFImageEncoder(model_path)
...
def evaluate(
    ...
    vectors, vector_failures = load_or_create_vectors(rows, assets, out, model_path, media_base_url, force_recompute)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate a local HuggingFace image encoder baseline.")
